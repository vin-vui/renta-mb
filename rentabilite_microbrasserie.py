...
@st.cache_data
def get_rentabilite_data(investissement_initial, cout_variable_par_unite, prix_vente_par_unite, couts_fixes_mensuels, quantites_produites, nb_mois, taxe_biere_par_litre, volume_alcoolique, cout_main_oeuvre, cout_marketing, cout_distribution, smic_mensuel):
    data = []
    for quantite in quantites_produites:
        ...
        for mois in range(1, nb_mois + 1):
            ...
            data.append([quantite, mois, cout_total, recettes, benefice, cout_taxe_alcool, cumul_benefice, rentabilite, remboursement_investissement, salaire_target])

    df = pd.DataFrame(data, columns=['Quantité Produite (unités/mois)', 'Mois', 'Coût Total (€)', 'Recettes (€)', 'Bénéfice (€)', 'Taxe Alcool (€)', 'Cumul Bénéfice (€)', 'Rentabilité', 'Remboursement Investissement', 'Salaire Visé'])
    return df
...
try:
    df = get_rentabilite_data(investissement_initial, cout_variable_par_unite, prix_vente_par_unite, couts_fixes_mensuels, quantites_produites, nb_mois, taxe_biere_par_litre, volume_alcoolique, cout_main_oeuvre, cout_marketing, cout_distribution, smic_mensuel)
